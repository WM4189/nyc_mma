...
function Train(props) {
  ...
  const filteredList = gyms
    .filter(
      (dojo) =>
        dojo.name.toLowerCase().includes(filter.toLowerCase()) ||
        dojo.art.toLowerCase().includes(filter.toLowerCase())
    )
    .sort((dojo1, dojo2) => {
  ...
  return (
    <>
      <h1 id="addGym">
        Explore or{" "}
        {
          <NavLink style={linkStyles} to="/new_gym">
            Add Gym
          </NavLink>
        }
      </h1>
